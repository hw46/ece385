...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE txBuf[] = {reg + 2, val};
	BYTE rxBuf[2];

	XSpi_SetSlaveSelect(&SpiInstance, 1); // Assuming slave select is 1
	int random = XSpi_Transfer(&SpiInstance, txBuf, rxBuf, 2);
	XSpi_SetSlaveSelect(&SpiInstance, 0);

	if (random != 0) {
		xil_printf("1 Error writing to MAX3421E register\r\n");
	}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int random = XSpi_Transfer(&SpiInstance, txBuf, rxBuf, nbytes + 1);
	XSpi_SetSlaveSelect(&SpiInstance, 0);


	if (random != 0) {
		xil_printf("2 Error writing to MAX3421E register\r\n");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	if (random != 0) {
		xil_printf("3 Error writing to MAX3421E register\r\n");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if (random != 0) {
		xil_printf("4 Error writing to MAX3421E register\r\n");
	}
